...
class OrchestrationTests(unittest.TestCase):
    def test_tasks_have_explicit_stage_names_and_scoped_retries(self):
        tasks = (
            ...
            orchestration.verify_loaded_data_task,
        )
        ...
        self.assertEqual([task.retries for task in tasks], [0, 0, 0, 0, 2, 2])
    ...
    def test_flow_calls_tasks_in_dependency_order(self):
        ...
        task_patches = {
            ...
            "verify_loaded": patch.object(
                orchestration, "verify_loaded_data_task", return_value=85
            ),
        }

        started = {name: patcher.start() for name, patcher in task_patches.items()}
        self.addCleanup(lambda: [patcher.stop() for patcher in task_patches.values()])
        ...
        self.assertEqual(
            ...
            [
                ...
                call.load("postgresql://test", 85),
                call.verify_loaded("postgresql://test", 85),
            ],
        )
